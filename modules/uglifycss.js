/**
 * A client side minifier based on Uglifycss
 */
    ...
    if (foundTerminator) {
      ...
      if (token.slice(0, 19) !== ___PRESERVED_TOKEN_) {
        ...
        if (
          options.convertUrls &&
          token.charAt(0) !== SEP &&
          token.slice(0, 7) !== "http://" &&
          token.slice(0, 8) !== "https://"
        ) {
...
function minifyCss(content = "", options = defaultOptions) {
  const comments = [];
  ...
  pattern = /(^|\})(([^\{:])+:)+([^\{]*\{)/g;
  content = content.replace(pattern, (token) =>
    token.replace(/:/g, "___PSEUDOCLASSCOLON___")
  );
  ...
  content = content.replace(
    /(^|[^.0-9\\])(?:0?\.)?0(?:ex|ch|r?em|vw|vh|vmin|vmax|cm|mm|in|pt|pc|px|deg|g?rad|turn|m?s|k?Hz|dpi|dpcm|dppx|%)/gi,
    "$10"
  );
